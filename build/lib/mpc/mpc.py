...
from .lqr_step import LQRStep, LQRStepGrad
...
class MPC(Module):
    ...
    def solve_lqr_subproblem(self, x_init, C, c, F, f, cost, dynamics, x, u,
        ...
        if self.slew_rate_penalty is None or isinstance(cost, Module):
            ...
            _lqr_grad_class = LQRStepGrad
            _lqr_grad = _lqr_grad_class.apply
            x, u = _lqr_grad(x_init, C, c, F, f if f is not None else e,  _lqr)
            return x, u, _lqr_grad_class.lqr_step
        else:
            ...
            _lqr_grad_class = LQRStepGrad
            _lqr_grad = _lqr_grad_class.apply
            x, u = _lqr_grad(_x_init, _C, _c, _F, _f, _lqr)
            x = x[:,:,self.n_ctrl:]
            return x, u,  _lqr_grad_class.lqr_step
